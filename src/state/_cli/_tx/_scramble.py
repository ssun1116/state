...
def add_arguments_scramble(parser: ap.ArgumentParser):
    """
...
def run_tx_scramble(args: ap.ArgumentParser):
    import logging
    import os
    import sys
    ...
    def scramble_weights(model, seed=None):
        ...
        for name, param in model.named_parameters():
            if param.requires_grad:
                ...
                param.data = scramble_tensor(param.data)
                #torch.randn_like(param.data)

        print("Weights scrambled!")

    def scramble_tensor(t: torch.Tensor) -> torch.Tensor:
        """
        Scrambles a tensor by shuffling its elements while keeping
        the same shape, mean, and std distribution.
        """
        flat = t.flatten()
        perm = torch.randperm(flat.numel(), device=t.device)
        scrambled = flat[perm].view(t.shape)
        return scrambled
    ...
    ############################################################
    # Add. Perform scrambling of randomized weights

    # for name, param in model.model.named_parameters():
    for name, param in model.named_parameters():
        print(f"{name}: {param.shape}, mean={param.data.mean():.4f}, std={param.data.std():.4f}")

    scramble_weights(model, seed = cfg["training"]["train_seed"]) # output message: Weight Scrambled!
